...
# Core Meraki context/config + lookups used for completion & summary
from .api import CFG, CTX, list_organizations, list_networks, get
...
def _autoload_actions():
    ...
    for m in pkgutil.iter_modules(actions_pkg.__path__):
        ...
        try:
            ...
            # Don’t crash help if a single action module fails to import.
...
# New category layout (only rendered if action exists)
_CATS = {
    "Org & Admin": {
        "admins","alerts","api usage","license","net summary",
        "traffic summary","security events","firmware"
    },
    "Session & Context": {
        "setup","ctx","reset ctx","use net","use org","whoami"
    },
    "Network": {
    ...
    "Configure": {
        "switch set","vlan add","wifi toggle"
    },
    "Inventory": {
        "overview","nets","orgs"
    },
...
def _grouped_actions():
    acts = list_actions()
    # Build groups with preferred categories, but only keep actions that exist
    groups = {k: [] for k in _CATS.keys()}
    for cat, names in _CATS.items():
        for nm in sorted(names):
            if nm in _HIDE_FROM_HELP: 
                continue
            if nm not in REGISTRY:
                continue
            meta = acts.get(nm, {})
            desc = _clean_desc(meta.get("desc","")) if isinstance(meta, dict) else _clean_desc(str(meta))
            groups[cat].append((nm, desc))
    # Also include any extra actions that weren’t listed in _CATS (fallback)
    listed = set().union(*_CATS.values())
    for nm, meta in acts.items():
        if nm in listed or nm in _HIDE_FROM_HELP or nm not in REGISTRY:
            continue
        desc = _clean_desc(meta.get("desc","")) if isinstance(meta, dict) else _clean_desc(str(meta))
        groups.setdefault("Other", []).append((nm, desc))
    # Sort each bucket
    for cat in groups:
...
def _print_actions():
    ...
    name_w, desc_w = 22, 64
    for cat in ("Org & Admin","Session & Context","Network","Configure","Inventory","Other"):
    ...
    print("  8) type 'help <action>' for usage & advanced flags\n")


# =============================================================================
# Mini dashboard summary (Option 1)
# =============================================================================
def _print_summary():
    """Compact summary: welcome, org/net, device counts, alerts (best-effort)."""
    try:
        user = CFG.get("user_name", "Engineer")
        org_name = CTX.org_name() or CFG.get("default_org_name") or "-"
        net_name = CTX.net_name() or CFG.get("default_network_name") or "-"
        print(Fore.CYAN + f" Welcome back, {user}!" + Style.RESET_ALL)
        print(f" Current Org: {Fore.YELLOW}{org_name}{Style.RESET_ALL}")
        print(f" Current Net: {Fore.YELLOW}{net_name}{Style.RESET_ALL}")

        # devices (best-effort)
        online = offline = total = 0
        if CTX.net_id():
            try:
                devs = get(f"/networks/{CTX.net_id()}/devices") or []
                if isinstance(devs, dict) and "devices" in devs:
                    devs = devs["devices"]
                total = len(devs)
                online = sum(1 for d in devs if (d.get("status") or "").lower() == "online")
                offline = sum(1 for d in devs if (d.get("status") or "").lower() == "offline")
            except Exception:
                pass
        print(f" Devices: {Fore.GREEN}{online} online{Style.RESET_ALL}, "
              f"{Fore.RED}{offline} offline{Style.RESET_ALL}, total {total}")

        # alerts (best-effort; endpoint may vary — fully guarded)
        alert_ct = None
        if CTX.org_id():
            try:
                # Some tenants expose /alerts; others /incidents. Try both safely.
                rows = get(f"/organizations/{CTX.org_id()}/alerts") or []
                if isinstance(rows, dict) and "items" in rows:
                    rows = rows.get("items") or []
                alert_ct = len(rows)
            except Exception:
                try:
                    inc = get(f"/organizations/{CTX.org_id()}/incidents") or []
                    if isinstance(inc, dict) and "items" in inc:
                        inc = inc.get("items") or []
                    alert_ct = len(inc)
                except Exception:
                    alert_ct = None
        if alert_ct is not None:
            print(f" Alerts: {alert_ct} (recent)")
    except Exception as e:
        print(Fore.RED + f" Summary unavailable: {e}" + Style.RESET_ALL)
    print()
...
def run(argv: List[str]):
    ...
    # interactive mode
    _print_actions()  # Option 2: reorganized categories
