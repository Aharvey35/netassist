...
_CATS = {
    "General": {"setup","ctx","reset ctx","use net","use org","whoami","license"},
    "Network": {"airmarshal","client find","devices status","events","mx status","mx vpn",
...
def _grouped_actions():
    acts = list_actions()
    groups = {k: [] for k in ("General","Network","Configure","Inventory")}
    ...
    return {k: groups.get(k,[]) for k in ("General","Network","Configure","Inventory")}

def _print_actions():
    print(Fore.CYAN + "\nAvailable Actions:" + Style.RESET_ALL)
    groups = _grouped_actions()
    name_w, desc_w = 22, 64
    for cat in ("General","Network","Configure","Inventory"):
